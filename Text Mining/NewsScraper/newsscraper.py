"""A script for scraping news sites and writing latest articles to json.
"""
...
def _handle_rss(company, value, count, limit):
    ...
    for entry in fpd.entries:
        ...
        # get news within one week since current day
        # now_time= datetime.now()
        today=datetime.today()
        today_timeframe= datetime.date(today)
        news_day = datetime.strptime(article["published"], '%Y-%m-%dT%H:%M:%S')
        news_day = datetime.date(news_day)
        delta = today_timeframe- news_day
        # print(news_day,today_timeframe, delta.days)
        if delta.days>30:
            continue
        try:
        ...
        print(f"{count} article(s) downloaded from {content.url}")
        count = count + 1
...
def _handle_fallback(company, value, count, limit):
    ...
    for content in paper.articles:
        ...
        try:
            ...
            print("continuing...")
            continue
        news_date=content.publish_date
        ...
        news_paper["articles"].append(article)
        print(
            f"{count} article(s) downloaded from {content.url}"
        )
        count = count + 1
        none_type_count = 0
    return count, news_paper
...
def main():
    ...
    """
    # sourcing from Google news portal with 3 key export countries(Peru, Chile, Norway) in Anvory
    args = list(sys.argv)

    if len(args) < 2:
        sys.exit("Usage: newsscraper.py News.json")
